...
void* receive(void* d)                   //replies from other clients..
{
int sockfd=*((int *)d);
int a;
char buf[256];
...
bzero(str,100);
printf("\nLOGGED IN USERS\n");
while(fscanf(file,"%s",str)!=EOF)             //reading username
{
if(str[0]=='u')
printf("\n%s\n",str);
fscanf(file,"%s",str);                        //reading password
}
...
{
sprintf(buffer,"%s|%s",buffer,user);           //chat message is send in the form 'To user-7:hello how are you|user-6' i.e client name follows |
